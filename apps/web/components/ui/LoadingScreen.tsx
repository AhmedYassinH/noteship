...
const LoadingScreen = ({
  ...
  const maskId = `noteship-loading-mask-${reactId}`;
  const isAr = activeLang === "ar";
  const copy = loadingCopy[activeLang];
  const surfaceCopy = copy.surfaces[surface];
  const messages = surfaceCopy.messages;
  const steps = surfaceCopy.steps;
  const currentMessage = state === "error" ? surfaceCopy.errorCopy : messages[messageIndex];
  const currentStep = state === "error" ? surfaceCopy.errorTitle : steps[messageIndex];
  const hasErrorAction = state === "error" && onErrorAction && surfaceCopy.actionLabel;
  ...
  const statusDots = useMemo(
    () => (
      <span className="ns-loading-dots" aria-hidden="true">
        <span />
        <span />
        <span />
      </span>
  ...
  return (
    <main
      className={cn("ns-loading-page font-body", className)}
      dir={isAr ? "rtl" : "ltr"}
      lang={activeLang}
      aria-labelledby="noteship-loading-title"
    >
      <section className="ns-loading-shell" aria-live="polite">
        <div className="ns-loading-mark-wrap" aria-hidden="true">
          <svg className="ns-loading-mark" viewBox="0 0 2048 2048" role="img">
            <defs>
              <path id={pathId} d={noteshipMarkPath} />
              <mask id={maskId}>
                <rect width="2048" height="2048" fill="#000" />
                <path
                  className="ns-loading-reveal-stroke ns-loading-stroke-left"
                  d="M 700 1380 C 765 1315 785 1230 775 1120 C 760 950 730 810 795 720 C 835 670 910 655 985 735"
                  fill="none"
                  pathLength={1}
                  stroke="#fff"
                  strokeLinecap="round"
                ...
                />
                <path
                  className="ns-loading-reveal-stroke ns-loading-stroke-middle"
                  d="M 805 1380 C 885 1260 830 965 880 870 C 945 755 1015 915 1080 1085 C 1150 1272 1195 1390 1260 1350"
                  fill="none"
                  pathLength={1}
                  stroke="#fff"
                  strokeLinecap="round"
                ...
                />
                <path
                  className="ns-loading-reveal-stroke ns-loading-stroke-right"
                  d="M 1140 1395 C 1265 1355 1310 1250 1302 1120 C 1292 965 1240 800 1305 710 C 1320 690 1338 678 1362 668"
                  fill="none"
                  pathLength={1}
                  stroke="#fff"
                  strokeLinecap="round"
            ...
            <use className="ns-loading-mark-ghost" href={`#${pathId}`} />
            <use className="ns-loading-mark-ink" href={`#${pathId}`} mask={`url(#${maskId})`} />
            <circle className="ns-loading-pen-tip" r="18">
              <animateMotion dur="2.55s" path={penTipPath} repeatCount="indefinite" />
            </circle>
          </svg>
        </div>

        <div className="ns-loading-wordmark">
          <h1 id="noteship-loading-title">{copy.title}</h1>
          <p>{currentMessage}</p>
        </div>

        <div className={cn("ns-loading-status", state === "error" && "ns-loading-status-error")}>
          <span>{currentStep}</span>
          {state === "loading" ? statusDots : null}
        </div>

        {state === "loading" ? (
          <div className="ns-loading-progress" aria-hidden="true">
            <span />
          </div>
        ) : null}

        {action || hasErrorAction ? (
          <div className="ns-loading-actions">
            {action}
            {hasErrorAction ? (
              <button className="ns-loading-primary-action" type="button" onClick={onErrorAction}>
                {surfaceCopy.actionLabel}
              </button>
            ) : null}
          </div>
        ) : null}
